...
with raw as (
    select u.id as upload_id, u.station_callsign, u.operator, qsos / n_operators as qsos,
        e.event, e.event ~ '^(CQ-WW|CQ-WPX|CQWW-?DIGI|(DARC-)?WAEDC|WAG)' as big,
        row_number() over (partition by operator, e.event ~ '^(CQ-WW|CQ-WPX|CQWW-?DIGI|(DARC-)?WAEDC|WAG)' order by qsos desc, u.id) as extra_contests
