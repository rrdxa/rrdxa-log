...
with log_with_op_time as (
    select call, band, coalesce(concat_ws('/', mode, submode), major_mode::text) mode, dxcc,
        tstzrange(
        case
...
def post_summary(data, summary, subject, send=True):
    ...
    if send:
        msg = EmailMessage()
        msg['From'] = Address(display_name=f"{data.uploader} via rrdxa.org",  addr_spec="noreply@rrdxa.org")
